 ...
 * examples that use DefaultTreeModel, see <a
 * href="http://java.sun.com/docs/books/tutorial/uiswing/components/tree.html"
 * >How to Use Trees</a> in <em>The Java Tutorial.</em>
 * <p>
...
public class MyTreeModel implements Serializable, TreeModel {
	...
	public MyTreeModel() throws RemoteException{
		this(new MyTreeNode("root"));
	}
	...
	public void valueForPathChanged(TreePath path, Object newValue) {
		...
		((MyTreeNode) aNode).setObject(newValue);
            try {
	...
	public void removeNodeFromExparent(MyTreeNode node) {
		MyTreeNode parent = (MyTreeNode) node.getOldParent();
	 ...
	 */
	public TreeNode[] getPathToRoot(MyTreeNode aNode) {
	 ...
	 *         specified node
	 */
	protected TreeNode[] getPathToRoot(MyTreeNode aNode, int depth) {
		TreeNode[] retNodes;
		...
		if (aNode == null) {
			...
			if (aNode == root)
				...
				retNodes = getPathToRoot((MyTreeNode) aNode.getParent(), depth);
	...
	private void readObject(ObjectInputStream s) throws IOException,
		...
		if (indexCounter < maxCounter
			...
			root = (MyTreeNode) values.elementAt(++indexCounter);
			indexCounter++;
		}
	}
